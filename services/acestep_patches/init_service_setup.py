...
class InitServiceSetupMixin:
    ...
    def _resolve_initialize_device(self, requested_device: str) -> str:
        ...
        Never silently resolves to "cpu": a GPU-tier model on CPU is not a
        degraded-but-working state, it's effectively hung (hours instead of
        seconds) with nothing in the API surface distinguishing it from a
        healthy GPU run. Any path that would have returned "cpu" now raises
        instead, so a lost CUDA device fails loudly at startup.
        """
        ...
        if device == "auto":
            ...
            if gpu_config.is_xpu_available():
                return "xpu"
            raise RuntimeError(
                "[initialize_service] No GPU backend (CUDA/MPS/XPU) detected. "
                "Refusing to silently initialize models on CPU."
            )
        ...
        if device == "cuda" and not gpu_config.is_cuda_available():
            ...
            if gpu_config.is_xpu_available():
                ...
                return "xpu"
            raise RuntimeError(
                "[initialize_service] CUDA requested but unavailable, and no other "
                "GPU backend (MPS/XPU) was found. Refusing to silently fall back to CPU."
            )
        ...
        if device == "mps" and not gpu_config.is_mps_available():
            ...
            if gpu_config.is_xpu_available():
                ...
                return "xpu"
            raise RuntimeError(
                "[initialize_service] MPS requested but unavailable, and no other "
                "GPU backend (CUDA/XPU) was found. Refusing to silently fall back to CPU."
            )
        ...
        if device == "xpu" and not gpu_config.is_xpu_available():
            ...
            if gpu_config.is_mps_available():
                ...
                return "mps"
            raise RuntimeError(
                "[initialize_service] XPU requested but unavailable, and no other "
                "GPU backend (CUDA/MPS) was found. Refusing to silently fall back to CPU."
            )
