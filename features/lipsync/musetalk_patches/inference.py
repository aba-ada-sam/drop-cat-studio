    ...
    for task_id in inference_config:
        try:
            ...
            for i, res_frame in enumerate(tqdm(res_frame_list)):
                ...
                out_png = f"{result_img_save_path}/{str(i).zfill(8)}.png"
                # CRITICAL: every index MUST get a PNG. Skipping (no face / bad
                # crop) leaves gaps in the %08d sequence, and ffmpeg's image2
                # reader STOPS at the first missing number -> the video collapses
                # to just the frames before the first gap (the "27 frames of 776"
                # near-frozen bug). For any skipped frame, write the ORIGINAL
                # frame so the sequence stays contiguous and full-length.
                if bbox == coord_placeholder:
                    cv2.imwrite(out_png, ori_frame)
                    continue
                x1, y1, x2, y2 = bbox
                if args.version == "v15":
                    y2 = y2 + args.extra_margin
                ...
                cv2.imwrite(out_png, combine_frame)
