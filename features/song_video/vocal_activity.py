"""Vocal-activity detection + gating for song-video audio conditioning.

RELOCATED 2026-08-05 during the MuseTalk removal. This module used to live at
features/lipsync/vocal_activity.py and served BOTH lip-sync paths the app had
(MuseTalk's post-pass and LTX-2's native audio conditioning). MuseTalk is gone
(Andrew's 2026-08-05 ruling -- see REVIEW_FINDINGS_2026-08-05.md LANDMINES),
but this module has zero MuseTalk-specific code in it -- it is pure
numpy/scipy/soundfile audio analysis -- so deleting it alongside the MuseTalk
package would have silently broken the native path's vocal isolation, which is
RATIFIED and must survive. Moved here verbatim (only this docstring changed)
rather than reimplemented, so behavior is provably unchanged.

A song is mostly instrumental. The native audio-conditioning path drives the
mouth from whatever energy is in the driving track. Even after Demucs isolates
the vocal stem, that stem still carries residual bleed -- cymbals, guitar,
kick -- through every instrumental bar. The model turns that bleed into mouth
movement, so the character sings to the background music instead of the
words.
...
expression lets a caller composite original frames back over the instrumental
stretches so the mouth is provably still.
...
def gate_audio(vocal_wav: str, intervals: list[tuple[float, float]], out_wav: str) -> bool:
    ...
    result is what native audio conditioning should be driven with: real
    silence between phrases instead of Demucs bleed, so the mouth rests when
    nobody is singing.
    """
    try:
